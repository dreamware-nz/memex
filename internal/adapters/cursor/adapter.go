...
// SettingsPath returns ~/.cursor/extensions/memex/hooks.json. If
...
func (a *Adapter) SettingsPath() string {
	...
	if err != nil {
		return filepath.Join(".cursor", "extensions", "memex", "hooks.json")
	}
	return filepath.Join(home, ".cursor", "extensions", "memex", "hooks.json")
}

// SkillsPath returns ~/.cursor/plugins/memex/skills, the
// install-anchored skills root for the Cursor adapter. We deliberately
// use ~/.cursor/plugins/ rather than ~/.cursor/extensions/ — the
// extensions directory is owned by the VS Code extension installer and
// Cursor never reads our skill markdown from there.
func (a *Adapter) SkillsPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return filepath.Join(".cursor", "plugins", "memex", "skills")
	}
	return filepath.Join(home, ".cursor", "plugins", "memex", "skills")
}
...
// matching the Claude Code shape that Cursor inherits from VS Code.
...
// ctx — the two hook types ctx cannot function without on Cursor.
