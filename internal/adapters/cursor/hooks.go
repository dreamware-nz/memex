...
// `memex hook ...` CLI dispatcher. Cursor reuses the Claude Code hook
// protocol verbatim (PreToolUse/PostToolUse/PreCompact/SessionStart)
// because Cursor is a VS Code fork sharing the same hook wire format.
const (
...
// "hooks" key of the Cursor hooks.json. Shape mirrors
// configs/cursor/hooks.json and the VS Code Copilot adapter so the
// install produces identical hooks.json files across the two forks.
