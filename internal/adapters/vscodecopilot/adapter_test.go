...
// clobber the developer's real ~/.vscode.
...
// 3.1 — Install on missing dir creates dir and hooks.json with expected keys.
func TestInstall_OnMissingDir_CreatesDirAndHooksJSON(t *testing.T) {
	home := setupHome(t)
	a := New()

	extDir := filepath.Join(home, ".vscode", "extensions", "memex")
...
// 3.3 — Uninstall removes ctx entries, preserves user entries.
...
// 3.4 — Validate returns "fail" for missing PreToolUse.
...
func TestSettingsPath_UsesVSCodeExtensionDir(t *testing.T) {
	home := setupHome(t)
	a := New()
	want := filepath.Join(home, ".vscode", "extensions", "memex", "hooks.json")
