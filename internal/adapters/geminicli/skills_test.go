...
func TestSkillsPath_Deterministic(t *testing.T) {
	...
	a := New()
	got := a.SkillsPath()
	want := filepath.Join(home, ".gemini", "plugins", "memex", "skills")
	if got != want {
...
func TestInstall_RollsBackHooksOnSkillsFailure(t *testing.T) {
	home := setupHome(t)
	blocker := filepath.Join(home, ".gemini", "plugins", "memex")
	...
	// Gemini stores hooks at top-level keys; check beforeTool isn't present.
	for _, k := range GeminiHookNames {
		if hookKeyPresent(parsed, k, testBinary) {
			t.Errorf("hook key %q not rolled back", k)
		}
	}
}
