...
// matcher becomes its own entry under settings.json's PreToolUse array
// so users can disable individual matchers without touching the others.
...
// hookCommand builds the canonical "memex hook <verb>" command string.
...
// hookBlock returns the full Claude Code hooks map ctx wants installed.
// Shape mirrors src/adapters/claude-code/index.ts so a user switching
// between the TS and Go builds doesn't end up with two flavours of
// settings.json drift.
