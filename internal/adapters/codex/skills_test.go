...
func TestSkillsPath_Deterministic(t *testing.T) {
	...
	a := New()
	got := a.SkillsPath()
	want := filepath.Join(home, ".codex", "plugins", "memex", "skills")
	if got != want {
...
func TestValidate_SkillsCheck(t *testing.T) {
	...
	if !found {
		t.Errorf("no 'skills' diagnostic in Validate output")
	}
}

func TestInstall_RollsBackHooksOnSkillsFailure(t *testing.T) {
	home := setupHome(t)
	blocker := filepath.Join(home, ".codex", "plugins", "memex")
	...
	settings, _ := os.ReadFile(a.SettingsPath())
	...
	if hookTypePresent(hooks, "PreToolUse", testBinary) {
		t.Errorf("hooks not rolled back: %s", string(settings))
	}
}
