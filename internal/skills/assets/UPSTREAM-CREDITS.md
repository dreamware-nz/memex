...
> **Snapshot**: 2026-05-07 from `memex/skills/` at commit `f34bc3e88f267c3320c421a00f0eb5700204c9db`.
> Updated by `ctx/internal/skills/assets/` snapshots only. Edit the upstream `memex/skills/` tree, then re-snapshot via the procedure documented in `openspec/changes/add-skills-and-installer/design.md` §Migration Plan.



memex vendors a small set of operating-discipline skills authored
by Matt Pocock. They are referenced as the operational backbone of the
[`memex-ops`](memex-ops/SKILL.md) skill (`/diagnose`, `/tdd`,
...
The owner operating directive at the top of `memex-ops/SKILL.md`
treats these skills as **mandatory tools**, not advisory references. If a
memex user invokes `/memex-ops` and the directive points to
skills they have never installed, the discipline collapses. Vendoring
guarantees that every install of memex ships the operational
