...
name: memex
description: |
  Use memex tools (memex_execute, memex_execute_file) instead of Bash/cat when processing
...
<context_mode_logic>
  <mandatory_rule>
    Default to memex for ALL commands. Only use Bash for guaranteed-small-output operations.
  </mandatory_rule>
</context_mode_logic>
...
**Everything else → `memex_execute` or `memex_execute_file`.** Any command that reads, queries, fetches, lists, logs, tests, builds, diffs, inspects, or calls an external service. This includes ALL CLIs (gh, aws, kubectl, docker, terraform, wrangler, fly, heroku, gcloud, etc.) — there are thousands and we cannot list them all.

**When uncertain, use memex.** Every KB of unnecessary context reduces the quality and speed of the entire session.
...
│   └── Use memex memex_execute or memex_execute_file
│
├── Fetching web documentation or HTML page?
│   └── Use memex_fetch_and_index → memex_search
│
├── Using Playwright (navigate, snapshot, console, network)?
│   └── ALWAYS use filename parameter to save to file, then:
│       browser_snapshot(filename) → memex_index(path) or memex_execute_file(path)
│       browser_console_messages(filename) → memex_execute_file(path)
│       browser_network_requests(filename) → memex_execute_file(path)
...
│   │   └── Use it directly. Do NOT re-index with memex_index(content: ...).
│   ├── Need to search the output multiple times?
│   │   └── Save to file via memex_execute, then memex_index(path) → memex_search
│   └── One-shot extraction?
│       └── Save to file via memex_execute, then memex_execute_file(path)
│
└── Reading a file to analyze/summarize (not edit)?
    └── Use memex_execute_file (file loads into FILE_CONTENT, not context)
```
...
|-----------|------|---------|
| Hit an API endpoint | `memex_execute` | `fetch('http://localhost:3000/api/orders')` |
| Run CLI that returns data | `memex_execute` | `gh pr list`, `aws s3 ls`, `kubectl get pods` |
| Run tests | `memex_execute` | `npm test`, `pytest`, `go test ./...` |
| Git operations | `memex_execute` | `git log --oneline -50`, `git diff HEAD~5` |
| Docker/K8s inspection | `memex_execute` | `docker stats --no-stream`, `kubectl describe pod` |
| Read a log file | `memex_execute_file` | Parse access.log, error.log, build output |
| Read a data file | `memex_execute_file` | Analyze CSV, JSON, YAML, XML |
| Read source code to analyze | `memex_execute_file` | Count functions, find patterns, extract metrics |
| Fetch web docs | `memex_fetch_and_index` | Index React/Next.js/Zod docs, then search |
| Playwright snapshot | `browser_snapshot(filename)` → `memex_index(path)` → `memex_search` | Save to file, index server-side, query |
| Playwright snapshot (one-shot) | `browser_snapshot(filename)` → `memex_execute_file(path)` | Save to file, extract in sandbox |
| Playwright console/network | `browser_*(filename)` → `memex_execute_file(path)` | Save to file, analyze in sandbox |
| MCP output (already in context) | Use directly | Don't re-index — it's already loaded |
| MCP output (need multi-query) | `memex_execute` to save → `memex_index(path)` → `memex_search` | Save to file first, index server-side |
| Wipe indexed KB content | `memex_purge(confirm: true)` | Permanently deletes all indexed content |

## Automatic Triggers

Use memex for ANY of these, without being asked:
...
- **Always use `queries` array** — batch ALL search questions in ONE call:
  - `memex_search(queries: ["transform pipe", "refine superRefine", "coerce codec"], source: "Zod")`
  - NEVER make multiple separate memex_search() calls — put all queries in one array

## External Documentation

- **Always use `memex_fetch_and_index`** for external docs — NEVER `cat` or `memex_execute` with local paths for packages you don't own
...
4. **For files you need to EDIT**: Use the normal Read tool. memex is for analysis, not editing.
5. **For Bash whitelist commands only**: Use Bash for file mutations, git writes, navigation, process control, package install, and echo. Everything else goes through memex.
6. **Never use `memex_index(content: large_data)`.** Use `memex_index(path: ...)` to read files server-side. The `content` parameter sends data through context as a tool parameter — use it only for small inline text.
...
<sandboxed_data_workflow>
  ...
  <workflow>
    LargeDataTool(filename: "path") → mcp__memex__memex_index(path: "path") → memex_search()
  </workflow>
</sandboxed_data_workflow>
...
```python
# FILE_CONTENT is pre-loaded by memex_execute_file
import json
...
Playwright `browser_snapshot` returns 10K–135K tokens of accessibility tree data. Calling it without `filename` dumps all of that into context. Passing the output to `memex_index(content: ...)` sends it into context a SECOND time as a parameter. Both are wrong.

**The key insight**: `browser_snapshot` has a `filename` parameter that saves to file instead of returning to context. `memex_index` has a `path` parameter that reads files server-side. `memex_execute_file` processes files in a sandbox. **None of these touch context.**
...
Step 2: memex_index(path: "/tmp/playwright-snapshot.md", source: "Playwright snapshot")
        → reads file SERVER-SIDE, indexes into FTS5, returns ~80B confirmation

Step 3: memex_search(queries: ["login form email password"], source: "Playwright")
...
Step 2: memex_execute_file(path: "/tmp/playwright-snapshot.md", language: "javascript", code: "
...
→ memex_execute_file(path: "/tmp/console.md", ...) or memex_index(path: "/tmp/console.md", ...)

browser_network_requests(includeStatic: false, filename: "/tmp/network.md")
→ memex_execute_file(path: "/tmp/network.md", ...) or memex_index(path: "/tmp/network.md", ...)
```
...
| `browser_snapshot()` → `memex_index(content: raw)` | **270K tokens** (doubled!) | NO |
| `browser_snapshot(filename)` → `memex_index(path)` → `memex_search` | **~430B** | YES |
| `browser_snapshot(filename)` → `memex_execute_file(path)` | **~250B** | YES |

### Key Rule

> **ALWAYS use `filename` parameter when calling `browser_snapshot`, `browser_console_messages`, or `browser_network_requests`.**
> Then process via `memex_index(path: ...)` or `memex_execute_file(path: ...)` — never `memex_index(content: ...)`.
>
> Data flow: **Playwright → file → server-side read → context**. Never: **Playwright → context → memex_index(content) → context again**.

## Subagent Usage

Subagents automatically receive memex tool routing via a PreToolUse hook. You do NOT need to manually add tool names to subagent prompts — the hook injects them. Just write natural task descriptions.

## Anti-Patterns

- Using `curl http://api/endpoint` via Bash → 50KB floods context. Use `memex_execute` with fetch instead.
- Using `cat large-file.json` via Bash → entire file in context. Use `memex_execute_file` instead.
- Using `gh pr list` via Bash → raw JSON in context. Use `memex_execute` with `--jq` filter instead.
- Piping Bash output through `| head -20` → you lose the rest. Use `memex_execute` to analyze ALL data and print summary.
- Running `npm test` via Bash → full test output in context. Use `memex_execute` to capture and summarize.
...
- Passing ANY large data to `memex_index(content: ...)` → data enters context as a parameter. **Always** use `memex_index(path: ...)` to read server-side. The `content` parameter should only be used for small inline text you're composing yourself.
- Calling an MCP tool (Context7 `query-docs`, GitHub API, etc.) then passing the response to `memex_index(content: response)` → **doubles** context usage. The response is already in context — use it directly or save to file first.
...
- Expecting `memex_stats` to reset or wipe anything → `memex_stats` is read-only (shows stats only). Use `memex_purge(confirm: true)` to permanently delete all indexed content.
