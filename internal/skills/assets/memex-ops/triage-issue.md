...
Use `memex_batch_execute` to gather everything in ONE call:
...
| **"Tool X sets env var Y"** | Find it in official source | `memex_fetch_and_index` the platform's docs/source. Grep their repo. If you can't find it, it probably doesn't exist. |
...
2. If `VERDICT: UNCONFIRMED` — do NOT implement. Instead, comment on the issue:
   ...
   - Debug output from: npx memex doctor (or memex-debug.sh)
...
**The `memex-debug.sh` script exists for exactly this purpose.** When in doubt, ask the reporter to run it and paste the output.
...
│   ├── UNCONFIRMED → Request evidence (memex-debug.sh output, repro steps)
