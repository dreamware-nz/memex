...
> Source: [mattpocock/skills/tdd](https://github.com/mattpocock/skills/tree/main/tdd) — embedded with memex enforcement.
...
### Go Toolchain Only — No Bundlers

**memex is a single Go module.** There are no `npm run build`, no JS bundles, no TypeScript compile step. Everything ships as one statically-linked Go binary. You run:

```bash
go test ./...   # validates behavior
go vet ./...    # static analysis
go build ./...  # confirms compilation
```

That's it. CI publishes release binaries; locally you only need the Go toolchain.
