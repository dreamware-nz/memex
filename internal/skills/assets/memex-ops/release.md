...
3. `version-sync.mjs` syncs version to ALL 6 manifest files:
   ...
   - `.pi/extensions/memex/package.json`
...
Verify publication:
```bash
npm view memex version  # should show new version
```
...
- [ ] Verify: `npm view memex version` shows new version
...
npm unpublish memex@{BAD_VERSION}

# Or deprecate
npm deprecate memex@{BAD_VERSION} "Known issue: {description}"
