...
Step 2: Run them locally (use memex_execute or a test)
Step 3: Record the ACTUAL output
Step 4: Compare actual vs. claimed behavior
Step 5: VERDICT:
  → REPRODUCED: Bug is real, proceed to fix
  → NOT_REPRODUCED: Ask reporter for memex-debug.sh output and exact repro steps
...
Step 2: Find HARD EVIDENCE — official docs, source code, or measured benchmarks
  → Use memex_fetch_and_index on official docs/repos
  → Use memex_execute to run actual tests
...
Could you provide:
1. Output from: `npx memex doctor` (or run `memex-debug.sh`)
...
```
Step 1: GREP — Does it exist in memex source?
  → rg "{ENV_VAR}" src/
...
```javascript
// WRONG — hardcoded /tmp
const tmpFile = "/tmp/memex-output.txt";

// CORRECT — uses OS temp dir
const tmpFile = path.join(os.tmpdir(), "memex-output.txt");
```
...
```shell
# File writing attempts through memex_execute
